...
A=diag(Weight)*basis(z1Q,n_basis_function);
...
% recording result in each iteration
f_test=zeros(n_basis_function,M);
AA=basis(zz,n_basis_function);
for i=1:n_basis_function
    for j=1:n_basis_function
        f_test(i,:)=f_test(i,:)+cc(j,i)*AA(:,j)';
    end
end

% total learning picture
p=1:M;
p=find(mod(p,100)==1);
figure 
plot(z(p),f(p),':');
hold on
for i=1:n_basis_function
    ff=(zmax-zmin)/(zzmax-zzmin)*f_test(i,:);
    plot(z(p),ff(p));
    hold on
end
hold off
